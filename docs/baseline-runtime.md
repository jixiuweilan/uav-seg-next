...
日期：2026-09-15，本页记录0.5.1基线交付；后续版本见[有限更新控制](training-control.md)。已补齐内部指标、审计样本读取、小型 U-Net、
单批次更新、完整图像预测及合成检查点保存/恢复代码。
0.5.0开发端64项合成测试通过；组员回报5项数值测试中4项通过、检查点保存失败。
修复文件流写入后，`b8ec92d` 全量5/5通过的[原始JSON已接收核验](checkpoint-fix.md)。退出码0另由组员回报。
...
后续0.6.0已补有限更新循环控制，但尚无正式训练入口、自动选优、断点续训或提交命令。
...
已有PyTorch2.8.0环境修复后全量5/5通过的原始报告已核验；不据此推断其他版本兼容性或正式训练性能。
