...
v2原始JSON已接收并核验，437字节，报告代码摘要与被测提交完全一致；
`suite=all`、`status=passed`、`tests_run=5`，失败、错误和跳过清单均为空。
文件SHA-256为 `9ea37ab80ed08fcfb626de597a5bd705edd69efe42e740ffb2e566311bb7cf16`，
被测代码摘要为 `c81d81474c8cea29daff70e475e56819235dcb0c2ea94349e1dac357b4157ab1`。
原文件保留，逐字节相同副本及接收回执存入本地忽略目录，未上传原始报告。
v1仍由组员保留，本次未转交；不再将v1接收作为这轮通过结果的前提。
该核验确认文件内容与被测代码一致，不等同于代理亲自观察执行过程。
[后续代码与接收方法](training-control.md)。
...
当时保留已完成的4项、不要求整轮重做；当时仅收到文字，原始JSON及其内容摘要尚未核验。
