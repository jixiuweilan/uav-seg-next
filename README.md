...
已补齐[基线模型代码与内部指标](docs/baseline-runtime.md)，修复版本数值测试5/5通过的原始JSON已接收并核对代码摘要。
已继续实现[有限更新循环与运行记录](docs/training-control.md)，尚无正式训练入口或训练结果。
最新完成[验证、确定性调度和合成选择管线](docs/validation-selection.md)，新增执行端控制测试待运行。
